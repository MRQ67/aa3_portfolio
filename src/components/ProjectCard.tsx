...
export function ProjectCard({
  ...
  const getTagIconSrc = (tag: string) => {
    ...
    switch (t) {
      ...
      case "partykit":
        return "/icons/partykit-icon.svg";
      case "yjs":
        return "/icons/yjs-icon.svg";
      case "convex":
        return "/icons/convex.svg";
      default:
        return null;
    }
  };
  ...
  const CardContent = (
    <div className="group relative w-full mx-auto space-y-4 font-[family-name:var(--font-dm-sans)]">
      ...
      <motion.div
        ...
        className="relative aspect-video md:aspect-[21/10] overflow-hidden rounded-xl group"
      >
        {/* Video for Coil Code Editor on hover */}
        {project.slug === 'coil-code-editor' ? (
          <>
            <Image
              src={project.image}
              alt={project.title}
              ...
              className="object-cover transition-all duration-500 rounded-xl opacity-100 group-hover:opacity-0"
            />
            <video
              src="/videos/Coil_code_demo.mp4"
              autoPlay
              loop
              muted
              playsInline
              className="absolute inset-0 w-full h-full object-cover rounded-xl opacity-0 group-hover:opacity-100 transition-all duration-500"
              onPlay={(e) => e.currentTarget.playbackRate = 1.0}
            />
          </>
        ) : (
          <Image
      ...
      >
        {(() => {
          ...
          const buttonSize = hasThreeButtons
            ? "px-3 py-2 text-xs"
            : "px-4 py-2.5 text-sm";
            ...
            <>
              ...
              {project.links.live && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`${buttonSize} bg-blue-500 hover:bg-blue-600 text-white rounded-full transition-all duration-300 flex items-center ${gapSize} font-[family-name:var(--font-dm-sans)]`}
                >
                  <svg
                    width={iconSize}
                    height={iconSize}
